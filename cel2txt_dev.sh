...
# calls R script to convert a single CEL file to TXT
	...
	echo $RBIN CMD BATCH --slave "--args celfile=$celFile outfile=$txtFile mas5method=$mas5method" $CEL2TXTCMD 
	$RBIN CMD BATCH --slave "--args celfile=$celFile outfile=$txtFile mas5method=$mas5method" $CEL2TXTCMD 
	gzip $txtFile	
...
#start and stop idx
	...
	run_cel2txt $celFile $txtFile $mas5method
    else
